...
function buildTable(reqData,pageNumber,pageSize,id) {
	 var url = urlRootContext()+"/get_user"; //请求的网址
     var reqParams = {'reqData':reqData, 'pageNumber':pageNumber,'pageSize':pageSize,'departmentId':id};//请求数据
     $(function () {
     	  $.ajax({
     	        type:"POST",
     	        url:url,
						 ...
						 if (dataList.length > 0 ) {
						     $(dataList).each(function(){//重新生成
						    	 ...
						    	 $(this.residentialQuarters).each(function(){
						    		 option = option + '<option value="'+this.id+'">'+this.rqName+'</option>';
						    	 });
						    	 
						         $("#tableBody").append('<tr id="'+ this.id +'">'+
						            		'<td ><div class="checkbox i-checks checkSingle"><label><input type="checkbox" value="'+ this.id +'" name="accountInfo"> <i></i></label></div></td>'+
						            		'<td >' + this.username + '</td>'+
						            		'<td >' + this.usercname + '</td>'+
						            		'<td >' + (this.sex == 1? "男":"女") + '</td>'+
						            		'<td >' + this.phone + '</td>'+
						            		'<td >' + this.email + '</td>'+
						            		/*'<td ><select id="rq_"'+this.userInfoId+' >'+
						            			option
					                         + '</select></td>'+*/
						            		/*'<td >' + (this.remark == null? "--":this.remark) + '</td>'+*/
						            		'<td ><button type="button" class="btn btn-primary btn-outline btn-xs repassword">Reset Password</button>'+
												'&nbsp;&nbsp;<button type="button" class="btn btn-primary btn-outline btn-xs modify">Update</button>'+
											'</td></tr>');
						         $("#"+this.id).data("userInfo",this);
						     	 });  
						     	$(".i-checks").iCheck({
...
function reloadEvent(){
	...
	$("#tableBody td .modify").click(function(event){
		accountId = $(this).parent().parent().attr("id");
		var userInfo = $("#"+accountId).data("userInfo");
		
		type = 2;//修改
		$("#accountEditModal #username").attr("readonly",true);
		$("#accountEditModal .modal-title").html("修改账号");
		$("#accountEditModal #username").val(userInfo.username);
		$("#accountEditModal #usercname").val(userInfo.usercname);
		$("#accountEditModal #email").val(userInfo.email);
		$("#accountEditModal #phone").val(userInfo.phone);
		$("#accountEditModal #priority").val(userInfo.priority);
		...
		//判断用户权限
		$("#accountEditModal #power").attr("disabled",false);
		$("#accountEditModal #power").parent().css("display","none");
		get_power("power",userInfo.role);
		//如果是超级用户admin不能修改角色
		if("admin"==(userInfo.username)){
			$("#accountEditModal #power").val('1');
			$("#accountEditModal #power").attr("disabled",true);
		}
	...
	$("#tableBody td .repassword").click(function(event){
		accountIdModify = $(this).parent().parent().attr("id");
		setDefaultValidator(passwordValidator);
		$("#passwordForm")[0].reset();
		var userInfo = $("#"+accountIdModify).data("userInfo");
		$("#modifyUsername").val(userInfo.username);
		$("#modifyPasswordModal").modal("show");
	});
}
//判断用户的权限
function get_power(id,power){
	$.ajax({
		type:"POST",
	        url:urlRootContext()+"/get_power",
	        data:{},
	        dataType:"json",
	        success: function(response){
	        	if(response!=null){
	        		if('superAdmin'==response.roles){
	        		
	        			//超级管理员
	        			$("#accountEditModal #power").parent().css("display","block");
	        			var role=response.rolesList;
	        			$("#"+id).empty();
	        			if(role.length>0){
	        				$(role).each(function(){
	        					$("#"+id).append('<option value="'+this.roleId+'" class="'+this.roleKey+'" >'+this.roleName+'</option>')
	        					
	        				});
	        				$(".plainUser").attr("selected",true);
	        				if(power!=null){
	        					$("#accountEditModal #power").val(power.roleId);
	        				}
	        			}
	        		}
	        	}
...
$("#addAccount").click(function(){
	type = 1;//新增
	$("#accountEditModal #username").attr("readonly",false);
	$("#accountEditModal .modal-title").html("新增账号");
	...
	$("#accountEditModal #usercname").val("");
	$("#accountEditModal #phone").val("");//email priority
	$("#accountEditModal #password").val("");
	$("#accountEditModal #confirmPassword").val("");
	$("#accountEditModal #email").val("");
	$("#accountEditModal #priority").val("");
	$("#accountEditModal input[name='sex'][value='1']").iCheck("check");
	//判断用户权限
	$("#accountEditModal #power").parent().css("display","none");
	$("#accountEditModal #power").attr("disabled",false);
	get_power("power");
...
$("#deleteAccount").click(function(){
	if(selectedArr.length != 0){
		swal({
			...
			$.ajax({
     	        type:"POST",
     	        url:urlRootContext()+"/delete_account_info/",
     	        traditional:true,
...
var saveAccountInfo = function(form){
    ...
    $.ajax({
		type:"post",
		url:urlRootContext()+"/save_account_info",
		traditional:true,
	    data:{"type":type,"accountId":accountId,"rqArr":rqArr,"username":$("#username").val(),
	    	"usercname":$("#usercname").val(),"power":$("#power option:selected").val(),
	    	"password":$("#password").val(),"email":$("#email").val(),"phone":$("#phone").val(),
	    	"priority":$("#priority").val(),"sex":sex},
		dataType:"json",
...
var savePassword = function(form){
    $.ajax({
		type:"post",
		url:urlRootContext()+"/modify_password?accountId="+accountIdModify,
		traditional:true,
	    data:$(form).serialize(),
		dataType:"json",
		success:function(data){
			if(data.success){
				swal({
					...
					if($("#modifyUsername").val()==$("#user_name").val()){
						//logout.do
