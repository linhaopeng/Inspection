...
function buildTable(reqData,pageNumber,pageSize,id) {
	 var url = urlRootContext()+"/get_department_page"; //请求的网址
						 ...
						 if (dataList.length > 0 ) {
						     $(dataList).each(function(){//重新生成
						    	 ...
						    	 $(this.residentialQuarters).each(function(){
						    		 option = option + '<option value="'+this.departmentId+'">'+this.rqName+'</option>';
						    	 });
						    	
						    	 
						         $("#tableBody").append('<tr id="'+ this.departmentId +'">'+
						            		'<td ><div class="checkbox i-checks checkSingle"><label><input type="checkbox" value="'+ this.departmentId +'" name="accountInfo"> <i></i></label></div></td>'+
						            		...
						            		'<td >'+
												'&nbsp;&nbsp;<button type="button" class="btn btn-primary btn-outline btn-xs modify">修改</button>'+
											'</td></tr>');
						         $("#"+this.departmentId).data("userInfo",this);
						     	 });  
						     	$(".i-checks").iCheck({
...
function reloadEvent(){
	...
	$("#tableBody td .modify").click(function(event){
		departmentId = $(this).parent().parent().attr("id");
		
		var userInfo = $("#"+departmentId).data("userInfo");
		type = 2;//修改
		$("#accountEditModal #username").attr("readonly",true);
		$("#accountEditModal .modal-title").html("修改部门");
		$("#accountEditModal #departmentName").val(userInfo.departmentName);
		$("#accountEditModal #describe").val(userInfo.describe);
		$("#accountEditModal #levels").attr("value",userInfo.levels);
		$("#accountEditModal #createdBy").val(userInfo.createdBy);
	...
	$("#tableBody td .repassword").click(function(event){
		departmentIdModify = $(this).parent().parent().attr("id");
		setDefaultValidator(passwordValidator);
		var userInfo = $("#"+departmentIdModify).data("userInfo");
...
$("#addAccount").click(function(){
	...
	$("#accountEditModal .modal-title").html("新增部门");
	$("#accountEditModal #departmentName").val("");
	
	$("#accountEditModal #describe").val("");
	$("#accountEditModal #departmentId").val("");
	$("#levels  option:selected").val("1");
...
$("#deleteAccount").click(function(){
	...
	if(selectedArr.length != 0){
		swal({
			...
			$.ajax({
     	        type:"POST",
     	        url:urlRootContext()+"/delete_department/",
     	        traditional:true,
     	        ...
     	        success: function(response){
     	            if(response.success) {
     	            	swal("已删除!", "你已删除选中的部门！", "success");
     	            	

     	            	//var reqData= {"departmentName":$("#departmentName").val()}; //取内容
     	            	//buildTable(JSON.stringify(reqData),(currentPage,PAGESIZE);//默认每页最多8条
     	            	//当选择删除的行等于总行，当前页-1
     	  ...
     	  $.ajax({
     	        ...
     	        success: function(response){
     	            if(response.success) {
						 ...
						 if (dataList.length > 0 ) {
						     $(dataList).each(function(){//重新生成
						            $("#rqTable #tableBody").append('<tr id="'+this.departmentId+'">'+
						            		'<td>'+
							        			'<div class="checkbox i-checks checkSingle" >'+
							        				'<label><input type="checkbox" value="'+this.departmentId+'" name="residentialQuarters"> <i></i></label>'+
							        			'</div>'+
							        		'</td>'+
							        		'<td>'+this.rqName+'</td>'+
...
//保存数据
var savedepartmen = function(form){
	console.log("submit!");
    ...
    $.ajax({
		type:"post",
		url:urlRootContext()+"/save_Deprtment",
		traditional:true,
	    data:{
	    	"type":type,
	    	...
	    	"levels":$('#levels option:selected').val(),
...
var savePassword = function(form){
    ...
    $.ajax({
		type:"post",
		url:urlRootContext()+"/modify_password?departmentId="+departmentIdModify,
		traditional:true,
	    data:$(form).serialize(),
		dataType:"json",
		success:function(data){
