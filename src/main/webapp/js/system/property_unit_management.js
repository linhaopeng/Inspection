...
function buildTable(reqData,pageNumber,pageSize) {
	 var url = urlRootContext()+"/get_property_units"; //请求的网址
...
function reloadEvent(){
	...
	//全选
	$("#propertyUnitTable .checkAll").on("ifChecked",function(event){
		var len = $("#propertyUnitTable #tableBody").find("input[type=checkbox]").length;
		var i = $("#propertyUnitTable #tableBody").find("input[type=checkbox]:checked").length;
		if(i != len){
			$("#propertyUnitTable #tableBody").find("input[type=checkbox]").iCheck("check");
		}
	});
	
	//反选
	$("#propertyUnitTable .checkAll").on("ifUnchecked",function(event){
		var len = $("#propertyUnitTable #tableBody").find("input[type=checkbox]").length;
		var i = $("#propertyUnitTable #tableBody").find("input[type=checkbox]:checked").length;
		if(i == len){
			$("#propertyUnitTable #tableBody").find("input[type=checkbox]").iCheck("uncheck");
		}
	});
	
	//单选
	$("#propertyUnitTable .checkSingle input").on('ifChecked', function(event) {
		if(true == $(this).is(':checked')){
			selectedArr.push($(this).val());//将所有选中的值添加进数组
			var len = $("#propertyUnitTable #tableBody").find("input[type=checkbox]").length;
			var i = $("#propertyUnitTable #tableBody").find("input[type=checkbox]:checked").length;
			if(len == i){
				$("#propertyUnitTable .checkAll").iCheck("check");
			}
		}
	});
	
	$("#propertyUnitTable .checkSingle input").on('ifUnchecked', function(event) {
		if(false == $(this).is(':checked')){
			selectedArr.splice($.inArray($(this).val(),selectedArr),1);//将所有选中的值从数组中删除
			var i = $("#propertyUnitTable #tableBody").find("input[type=checkbox]:checked").length;
			var len = $("#propertyUnitTable #tableBody").find("input[type=checkbox]").length;
			if(i != len){
				$("#propertyUnitTable .checkAll").iCheck("uncheck");
			}
		}
	});
	...
	$("#tableBody td .editCompany").click(function(event){
		...
		$("#companyEditModal .modal-title").html("编辑物业单位");
...
var saveCompanytInfo = function(form){
    ...
    $.ajax({
		type:"post",
		url:urlRootContext()+"/save_company_info?type="+type+"&companyId="+companyId+"&companyType=3",
		traditional:true,
	    data:$(form).serialize(),
		dataType:"json",
